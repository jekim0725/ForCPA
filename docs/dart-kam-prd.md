# DART 핵심감사사항(KAM) 추출 도구 PRD v1.0

> 상태: 구현 준비(기술 스파이크 통과 전)  
> 대상 릴리스: MVP  
> 최종 수정일: 2026-07-20

## 0. 한 줄 요약

상장기업명 또는 6자리 종목코드를 입력하면 DART의 최신 유효 사업보고서에서 감사보고서를 식별하고, **연결감사보고서를 우선으로 핵심감사사항(KAM) 원문과 감사인의 대응을 항목별로 추출**해 원문 링크와 함께 보여주는 Streamlit 앱이다.
...
4. 앱이 KAM 항목, 선정 이유, 감사인의 대응을 원문 그대로 표시한다.
5. 사용자가 DART 원문 링크를 열어 결과를 대조한다.
...
KAM은 접을 수 있는 항목 카드로 표시하며 제목, 선정 이유, 감사인의 대응, 전체 원문을 제공한다. 원문 복사 기능은 줄바꿈을 보존한다.
...
- 모바일 최적화보다 데스크톱에서 긴 원문을 읽고 복사하기 좋은 레이아웃을 우선한다.
